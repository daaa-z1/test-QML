...
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            // Mengatur sudut awal dan akhir lingkaran agar 70% lingkaran
            var startAngle = Math.PI + (Math.PI * 0.15);
            var endAngle = 2 * Math.PI - (Math.PI * 0.15);
            ...
            for (var i = minValue; i <= maxValue; i += (maxValue - minValue) / 10) {
                var angle = startAngle + (i - minValue) / (maxValue - minValue) * (endAngle - startAngle);
            ...
            // Draw the needle.
            var valueAngle = startAngle + (value - minValue) / (maxValue - minValue) * (endAngle - startAngle);
            ctx.beginPath();
