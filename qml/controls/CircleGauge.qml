...
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            ctx.beginPath();
            ctx.arc(width / 2, height / 2, width / 2 - 2, Math.PI * 0.65, Math.PI * 1.35, false);
            ctx.lineWidth = 4;
            ...
            for (var i = minValue; i <= maxValue; i += (maxValue - minValue) / 10) {
                var angle = i * Math.PI * 0.7 / (maxValue - minValue) + Math.PI * 0.65;
            ...
            // Draw the needle.
            var valueAngle = valueItem.value * Math.PI * 0.7 / (maxValue - minValue) + Math.PI * 0.65;
            ctx.beginPath();
