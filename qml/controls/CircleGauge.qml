...
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            var centerX = width / 2;
            var centerY = height * 0.5; // Membuat lingkaran bagian bawah
            var radius = Math.min(centerX, centerY) - 10;

            // Draw the background arc.
            ctx.beginPath();
            ctx.arc(centerX, centerY, radius, Math.PI * 0.5, Math.PI * 2.5, false); // Menggambar lingkaran 70%
            ...
            for (var i = minValue; i <= maxValue; i += (maxValue - minValue) / 10) {
                var angle = (i - minValue) / (maxValue - minValue) * Math.PI * 1 + Math.PI * 0.5; // Sesuaikan dengan lingkaran 70%
            ...
            // Draw the needle.
            var valueAngle = (value - minValue) / (maxValue - minValue) * Math.PI * 1 + Math.PI * 0.5; // Sesuaikan dengan lingkaran 70%
            ctx.beginPath();
