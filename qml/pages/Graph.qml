...
Page {
    ...
    //         var key = keys[i];
    //         if (key) {
    //             var value = mainApp.value[keys[i].name];
    //             lineSeries[i].append(currentTime, value);
    //         }
    //         console.log(mainApp.value[keys[i]]);
    //     }
    // }
    ...
    function startNextTest() {
        if (testQueue.length > 0) {
            ...
            chartView = currentTest

            for (var i = 0; i < keys.length; i++) {
                var key = keys[i];
                testData[currentTest].push(mainApp.value[key]);
                lineSeries[i].append(currentTime, mainApp.value[key]);
            }
